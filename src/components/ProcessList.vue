...
// const targetProcess = ["/snap/postman/184/usr/share/postman/postman", "msedge.exe", "retrodtc", "falldtc"];
  ...
  methods: {
    ...
    async handleRunShell(type = "start") {
      ...
      if (res) {
        ...
        let resData = null
        if (type === "start")
          resData = await API.processStart(names)
        else
          resData = await API.processStop(names)
